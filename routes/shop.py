...
from services.shop_service import navigate_to_shop_daily_schedule, download_excel_for_shop_date
from services.shop_parser import parse_shop_excel
...
async def get_shop(
    dates: List[str] = Query(
        ..., description="조회할 샵 날짜 리스트 (예: ?dates=2025-12-10&dates=2025-12-15)")
):
    driver = None
    try:
        clean_dirs()
        ...
        for curr in target_dates:
            ...
            f = download_excel_for_shop_date(driver, curr)
            if f:
                data = parse_shop_excel(f, d_str)
