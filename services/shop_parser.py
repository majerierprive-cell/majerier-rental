...
def parse_shop_html_xls(path, date):
...
def parse_shop_excel(path, date):
    try:
        # Fake Excel Check
        with open(path, 'rb') as f:
            if b'<!DOCTYPE' in f.read(20) or b'<html' in f.read(20):
                return parse_shop_html_xls(path, date)

        # 실제 XLS/XLSX는 기존 로직 유지 (생략)
        # 하지만 보통 Fake Excel이므로 여기로 넘어오는 경우는 드뭄
        return parse_shop_html_xls(path, date)  # 일단 HTML 파서로 보냄 (안전장치)
